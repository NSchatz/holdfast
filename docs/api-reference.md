...
| `output_width`, `output_height` | any job whose encoder produced a file | the OUTPUT's pixel dimensions, measured on the file the encoder wrote and recorded whether the gates then accepted it or rejected it. This build **downscales nothing**, so they are the source's own dimensions on an ordinary job - which is what makes a row where they are not a fact worth having. `null` when no output was produced |
