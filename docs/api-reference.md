...
| `undetermined-source-height` | this root needs the source's height - it bands its thresholds by it, or it sets a `max_height` output ceiling - and the probe could not establish one |
| `downscale-unacknowledged` | this file would be scaled down by `max_height`, the undo window is disabled so the swap would be final, and the root did not set `downscale_acknowledged`. Set that key, or set `undo_window_hours` |
...
| `reason` | skipped | **which guard** fired - `already-at-target-codec`, `low-bitrate`, `hardlinked`, `symlinked-source`, `interlaced`, `dolby-vision`, `hdr10-plus`, `incomplete-hdr-metadata`, `exotic-pixel-format`, `multi-video-stream`, `unreadable-stream-list`, `undetermined-source-height`, `downscale-unacknowledged`, `unknown-field-order`, `telecine-cadence`, `target-already-exists`, `undo-retention-failed`, `restored-original`, `operator-excluded` |
...
| `vmaf_scored_width`, `vmaf_scored_height` | a job that scaled and was then scored | the resolution the **perceptual comparison was made at**, which for such a job is the SOURCE's: the output is scaled back UP to meet the source and scored there, against the source exactly as it is, so the figures carry what the downscale cost. Scoring against a source resampled DOWN to meet the output would hide it, and nothing else on this row distinguishes those two numbers. `null` on every job that scaled nothing, whose comparison was made at the size `output_width`/`output_height` already state |
...
| `selection_not_applied` | a job whose selection was not applied in part | today only `audio-selection-would-leave-no-audio`: applying the audio selection would have produced a file with **no audio at all**, so every audio stream was carried forward instead. Absent when the selection applied |
| `vmaf_skipped` | a job whose perceptual gate did not run | today only `remux-only-video-identical`: the job was a `remux_only` remux and every video stream it carries was established **identical** to the source's before the gate was declined. Such a row carries **no VMAF figure of any kind** - not a zero, and not a model, format or stream either |
