...
`rescan`, `pause`, `resume`, the ledger search (`/api/search`) and the withheld paths
(`/api/exclusions`) answer **403** to every caller - a safe default, not a broken one, and
the dashboard and the read API still work. The ledger search is in that group and not
among the unauthenticated reads for a reason worth stating: the capped reads ship at most
a few hundred rows, so a search over the whole ledger serves per-file rows they have never
served, and gating it keeps this a control-gated read rather than a new unauthenticated
one. A proxy identity header (`Remote-User`,
