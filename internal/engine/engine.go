...
func (e *Engine) ProcessFile(ctx context.Context, f string) error {
	...
	// NOTE (scope): the HDR/DV guard (detect-and-skip Dolby Vision / HDR10+, carry
	// HDR10 static metadata) is TRANSCODE-3. Until then this CPU path re-encodes
	// colour/HDR sources without colour propagation — TRANSCODE-3 adds that guard
	// before the tool is pointed at a real HDR library.

	dir := filepath.Dir(f)
	base := filepath.Base(f)
	stem := strings.TrimSuffix(base, filepath.Ext(base))
	tmp := filepath.Join(dir, stem+"."+TempMarker+"."+e.Cfg.ContainerExt)
	final := filepath.Join(dir, stem+"."+e.Cfg.ContainerExt)
